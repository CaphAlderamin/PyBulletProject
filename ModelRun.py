...
from Attendants import *
device = select_device()
resize = get_resize()

writer = SummaryWriter()
i_episode = 0
def collect_trajectories(envs, policy, num_agents, action_size, tmax=200, nrand=5,):
...
def eval_policy(envs, policy, action_size, tmax=1000):
    reward_list=[]
    state = envs.reset()
    for t in range(tmax):
        states = get_screen(envs)
        action_est, values = policy(states)
        sigma = nn.Parameter(torch.zeros(action_size))
        dist = torch.distributions.Normal(action_est, F.softplus(sigma).to(device))
        actions = dist.sample()
